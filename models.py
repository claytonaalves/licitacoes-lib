...
class Licitacao(BaseModel):
    ...
    def filtrar_por(**kwargs):
        ...
        if kwargs['credenciamento_inicio'] and kwargs['credenciamento_fim']:
            select = select.where(Licitacao.prazo_credenciamento.between(
                                    formata_data(kwargs['credenciamento_inicio']), 
                                    formata_data(kwargs['credenciamento_fim'])))
        ...
        if kwargs['objeto']:
            select = select.where(SQL("match (objeto) against ('{}')".format(kwargs['objeto'])))
